...
from FeatureExtractor.CENAExtractor import CenaExtractNodeFeature
from Methods.GetAlignments import KDTreeAlignmentHit1, KDTreeAlignmentHitK, MAP, AUC, MNC, EMNC
from tools import evaluate, read_tex_graph, create_align_graph, \
    shuffle_graph, get_top_k_acc, greedy_match, compute_structural_similarity, jaccad, get_graph_degree_feature, \
    print_run_time

from Methods.RwBasedMethods.RWEmbeddingMethods.DW import netmf
...
def align_embedding(dataname,g1,g2 ,nodes1 ,nodes2,K_nei,r_rate,e1=None,e2=None):
...
degree_thresold = 6
@print_run_time
def run_mmnc_align(dataname,g1,g2,ans_dict,K_de=3,K_nei=3,metric=[],train_ratio=0.04,r_rate=0,fast=False):
    ...
    aligned_embed1,embed2,trans_combined_e1,combined_e2 = align_embedding(dataname,g1,g2,nodes1,nodes2,K_nei,r_rate)
    E3 = time()
...
@print_run_time
def run_immnc_align(dataname,g1,g2,ans_dict,K_de=3,K_nei=3,
    ...
    for i in range(niters):
        ...
        if i==0:
            ...
            aligned_embed1, embed2, trans_combined_e1, combined_e2 = align_embedding(dataname,g1, g2, nodes1,
    ...
    if "hits5" in metric:
        acc = KDTreeAlignmentHitK(aligned_embed1, embed2, ans_dict, K=5)
        print("iMMNC, acc_hits@5:{}".format(acc))
    if "AUC" in metric and "MAP" in metric:
        matrix = euclidean_distances(aligned_embed1, embed2)
        matrix = np.exp(-matrix)
        negative_num = int((1 - Acc) * aligned_embed1.shape[0])
        map = MAP(matrix, ans_dict)
        auc = AUC(matrix, ans_dict, negative_num)
        print("iMMNC, MAP:{},AUC:{}".format(map, auc))
    if "MNC" in metric:
        aligments = euclidean_distances(aligned_embed1, embed2)
        values = np.argmax(aligments, axis=-1).tolist()
        pred_dict = dict(zip(list(range(len(values))), values))
        mnc = MNC(pred_dict, g1, g2)
        print("iMMNC, MNC:{}".format(mnc))
    if "EMNC" in metric:
        aligments = euclidean_distances(aligned_embed1, embed2)
        values = np.argmax(aligments, axis=-1).tolist()
        pred_dict = dict(zip(list(range(len(values))), values))
        emnc = EMNC(pred_dict, aligned_embed1, embed2, g1, g2)
        print("iMMNC, EMNC:{}".format(emnc))
        #
